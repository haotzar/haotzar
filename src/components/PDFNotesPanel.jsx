...
const PDFNotesPanel = ({ bookName, currentPage, onClose, autoOpenCreate = false, initialContent = '' }) => {
  ...
  const [panelWidth, setPanelWidth] = useState(() => {
    const saved = localStorage.getItem('pdfNotesPanelWidth');
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (newWidth >= 200 && newWidth <= 600) {
        ...
        localStorage.setItem('pdfNotesPanelWidth', newWidth.toString());
      }
    };
  ...
  return (
    <div className="pdf-notes-panel-sidebar" ref={panelRef} style={{ width: `${panelWidth}px` }}>
      {/* Resize Handle */}
